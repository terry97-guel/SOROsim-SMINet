%Function now ouputs center point of mesh
%Function now take FalceAlpha as optional parameter

function centers = plotqGetPosition(S,q,varargin)

% Parse options
ps = inputParser;
addParameter(ps,'FaceAlpha',1);

parse(ps,varargin{:});

FaceAlpha             = ps.Results.FaceAlpha;
...
for i=1:N
    ...
    if S.VLinks(i).linktype=='r'
        ...
        g_here     = g_here*g0f;
%         patch(Xpatch,Ypatch,Zpatch,color,'EdgeColor','none','FaceAlpha',FaceAlpha);
    end
    ...
    for j=1:(S.VLinks(i).npie)-1
        ...
        for ii=1:n_l-1
            ...
            y_here     = pos_here(2,:);
            z_here     = pos_here(3,:);
            center     = [x_here(1)+x_here(2), y_here(1)+y_here(2),z_here(1)+z_here(2)];
            center     = center/2;
